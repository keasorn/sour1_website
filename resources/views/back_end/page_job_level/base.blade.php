<div class="col-md-12 margin-25-0">
    <table class="custom-table-m table">
        ...
        @foreach($jobLevels as $row)
            ...
            <tr>
                ...
                <td class="page-header">
                    ...
                    <form class="row" method="POST"
                          action="{{ route('job-level.destroy', ['id' => $row->id]) }}"
                        ...
                        <a href="{{ route('job-level.edit', ['id' => \App\Http\Controllers\MyUtility::my_encrypt($row->id,true)]) }}"
            ...
            <?php
            $rst = clone $users;
            $uss = $rst->where("job_level", $row->id)->get();
            ?>
            @foreach($uss as $rw)
